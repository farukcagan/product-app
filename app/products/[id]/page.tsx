...
export default function ProductDetailPage() {
    ...
    useEffect(() => {
        ...
        if (id) {
            ...
            productService.getCommentsByProductId(id as string)
    ...
    if (!product && !loading) return (
        <div className="min-h-screen bg-white">
            ...
            <div className="flex flex-col items-center justify-center h-[calc(100vh-80px)]">
                <h1 className="text-2xl font-bold text-brand-secondary">{CONTENT.productDetail.notFound}</h1>
                <Button
                    variant="ghost"
                    onClick={() => router.push('/products')}
                    className="mt-4 !text-brand-primary !font-bold hover:underline !bg-transparent"
                >
                    {CONTENT.productDetail.goBack}
                </Button>
            </div>
        </div>
    );
            ...
            <main className="mx-auto max-w-[1400px] px-6 lg:px-10 py-12 lg:py-20">
                <div className="flex flex-col mt-12 gap-12 lg:gap-20 lg:flex-row">
                    ...
                    <div className="flex flex-1 flex-col">
                        ...
                        <div className="mb-8">
                            <h3 className="mb-4 text-sm font-bold text-brand-secondary">{CONTENT.productDetail.selectColor}</h3>
                            <div className="flex gap-4">
                                <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-white p-3 pr-6 shadow-sm">
                                    ...
                                    <span className="text-[13px] font-bold text-gray-400">{CONTENT.productDetail.colorSilver}</span>
                                </div>
                                <div className="flex items-center gap-3 rounded-xl border border-brand-primary/30 bg-white p-3 pr-6 shadow-sm ring-4 ring-brand-primary/5">
                                    ...
                                    <span className="text-[13px] font-bold text-brand-secondary">{CONTENT.productDetail.colorBlack}</span>
                        ...
                        <div className="mb-12">
                            <h3 className="mb-4 text-sm font-bold text-brand-secondary">{CONTENT.productDetail.selectFeature}</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="relative rounded-2xl border-2 border-brand-primary bg-white p-5 shadow-sm">
                                    ...
                                    <p className="mb-2 text-[13px] font-bold text-brand-secondary">{CONTENT.productDetail.featureTitle}</p>
                                    <p className="text-[12px] text-brand-text-muted leading-tight">{CONTENT.productDetail.featureDescription}</p>
                                </div>
                                {[...Array(3)].map((_, i) => (
                                    <div key={i} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm opacity-60">
                                        <p className="mb-2 text-[13px] font-bold text-gray-400">{CONTENT.productDetail.featureTitle}</p>
                                        <p className="text-[12px] text-gray-300 leading-tight">{CONTENT.productDetail.featureDescription}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            <div className="mb-8 flex items-center justify-between">
                                <h3 className="text-lg font-bold text-brand-secondary">{CONTENT.productDetail.commentsTitle}</h3>
                            </div>
                                        ...
                                        {comments.map((comment) => (
                                            <div key={comment.id} className="group">
                                                ...
                                                <p className="text-[14px] text-brand-text-muted leading-relaxed">
                                                    {comment.body}
                                                    <Button variant="ghost" className="!p-0 ml-2 !font-bold !text-brand-primary hover:underline !inline-flex !bg-transparent h-auto w-auto !min-h-0 !min-w-0">{CONTENT.productDetail.showMore}</Button>
                                                </p>
                                            </div>
                                        ))}
                                        <Button className="mt-4 !bg-brand-secondary !rounded-xl !px-10 !h-12 !font-bold">
                                            {CONTENT.productDetail.viewAll}
                                        </Button>
                                    </>
                                ) : (
                                    <div className="py-10 text-center text-gray-400 italic bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                        {CONTENT.productDetail.noComments}
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <div className="fixed bottom-0 left-0 flex h-[120px] w-full items-center border-t bg-white px-6 lg:px-12 z-50">
                <div className="mx-auto flex w-full max-w-[1400px] items-center">
                    <div className="hidden lg:flex w-[200px] h-[120px] items-center border-r border-gray-100 mr-12">
                        <h2 className="text-2xl font-black text-brand-secondary tracking-tight">{CONTENT.productDetail.orderSummary}</h2>
                    </div>
